#Modified from https://github.com/meetshah1995/tf-3dgan
...
LOCAL_PATH = '/home/ankbzpx/3DShapeNets/volumetric_data/'
...
def getVFByMarchingCubes(voxels, threshold=0.5):
    verts, faces, normals, values =  sk.marching_cubes_lewiner(voxels, level=threshold)
    return verts, faces, normals, values

def plotMeshFromVoxels(voxels, threshold=0.5):
    v,f,_,_ = getVFByMarchingCubes(voxels, threshold)
    plotFromVF(v,f)

def plotVoxelVisdom(voxels, visdom, title, threshold=0.5):
    v,f,_,_ = getVFByMarchingCubes(voxels, threshold)
